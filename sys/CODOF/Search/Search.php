...
class Search
{
    ...
    public function search()
    {
        ...
        if (!empty($this->cats)) {
            $this->values = array_merge($this->values, $this->cats);
        }
        $values = $this->values;
    ...
    public function get_matching_str($str)
    {
        ...
        $tagStrings = [];
        // For deleting DOM elements you MUST iterate backwards -_-
        for ($i = $tags->length; --$i >= 0;) {
            $tag = $tags->item($i);
            if (strpos($tag->getAttribute('class'), "codo_quote_container") !== FALSE) {
                $tagStrings[] = $tag->C14N();
    ...
    private function getSelectors()
    {
        ...
        return 'p.post_id, p.omessage AS message,p.imessage, p.post_created,p.post_modified,p.reputation,last_post_id, '
            . 'u.id, u.username as name, u.avatar, u.signature,r.rid, c.cat_img, c.cat_alias, c.cat_name, t.topic_created,'
            . 't.topic_id, t.post_id AS topic_post_id, t.uid, t.title,t.cat_id, t.no_posts, t.no_views, t.last_post_time, '
    ...
    private function base_query()
    {
        ...
        if ($this->cats != null) {
            $in = str_repeat('?,', count($this->cats) - 1) . '?';
            $qry .= " AND p.cat_id IN ($in) ";
            //var_dump($qry);
            //exit();
        }
