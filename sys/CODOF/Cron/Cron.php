...
class Cron extends Jobs
{
    ...
    public function run($cron = null)
    {
        ...
        foreach ($crons as $cron) {
            ...
            //errors .
            try {
                \CODOF\Hook::call('on_cron_' . $cron['cron_name'], $cron);
            } catch (Exception $ex) {
     ...
     * @param int $timestamp interval after which cron will run
     */
    ...
    public function remove($name)
    {
        ...
        return $obj->execute(array("name" => $name));
    }
    ...
    private function cleanUp()
    {
        ...
        if (!isset($_SESSION['codo_zombie_crons_cleanup']) || (int)$_SESSION['codo_zombie_crons_cleanup'] < (time() - 300)) {

            $_SESSION['codo_zombie_crons_cleanup'] = time();
            $hour_old = time() - 300;
