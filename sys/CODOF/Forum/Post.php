...
class Post extends Forum
{
    ...
    /**
     * Insert attachment row in DB. Used for mapping between real name and actual filename
     * @param $attachment
     * @return array
     */
    public static function saveAttachmentInDB($attachment)
    {
        $visibleHash = \uniqid();
        ...
        if (self::getMimeType($attachment['tmp_name']) == "video/mp4") {
            $visibleHash .= ".mp4";
            $storedHash = $visibleHash;
        }
    ...
    /**
     * Resizes the image to save a preview
     * @param $storedHash
     * @param int $width
     * @throws ImageResizeException
     */
    public static function saveAttachmentPreview($storedHash, $width = 683)
    {
    ...
    {
        $hashes = self::saveAttachmentInDB($attachment);
        $visibleHash = $hashes[0];
        $storedHash = $hashes[1];
        ...
        self::saveAttachmentPreview($storedHash);
    ...
    public function gen_posts_arr($posts, $search = false)
    {
        ...
        foreach ($posts as $post) {
            ...
            $_posts[$i] = array(
                ...
                "message" => $message,
                "imessage" => htmlspecialchars($post['imessage'], ENT_QUOTES, "UTF-8"),
        ...
        $badges = $badgeService->getRewardedBadgesForUsers($uids);
        foreach ($_posts as $key => $topic) {
            $_posts[$key]['badges'] = [];
            if (!isset($badges[$topic['id']])) continue;
            foreach ($badges[$topic['id']] as $badge) {
                $_posts[$key]['badges'][] = [
                    "name" => $badge->name,
                    "badgeLocation" => $badge->badgeLocation,
                    "hasDescription" => $badge->description != "", // Spoon feed Handlebars :(
                    "description" => $badge->description
                ]; // Handlebars in LightnCandy requires normal array :(
            }
        }
