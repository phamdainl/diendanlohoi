...
class Notifier
{
    ...
    /**
     * Usually notifications are first queued in codo_notify_queue and dequeued based on subscribers
     * Sometimes we do not need to go through this when we know there can be only one subscriber per
     * notification i.e a direct notification like awarding badges
     * TODO: Move vote up/down to direct notifications
     * @param $type
     * @param $data
     * @param $uid
     */
    public function directNotify(string $type, array $data, int $uid)
    {
        //Insert notification data JSON encoded
        $nid = \DB::table(PREFIX . 'codo_notify_text')->insertGetId(
            array("data" => json_encode($data), "status_link" => -1)
        );
        $this->notify([$uid], $type, $nid);
    }
