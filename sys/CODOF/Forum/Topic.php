...
class Topic extends Forum
{
    ...
    public function get_topics($cat_id, $page = 1)
    {
        ...
        $qry = 'SELECT p.reputation,p.post_id,p.omessage AS message, p.post_created, r.rid, u.id, u.name AS name, u.avatar, ' .
            ...
            ' t.topic_status=' . Forum::STICKY_ONLY_CATEGORY_CLOSED . '  ) AND ' . $this->getViewTopicPermissionConditions(true) .
            'ORDER BY ' .
            ' CASE t.topic_status ' .
            ' WHEN ' . Forum::STICKY_CLOSED . ' THEN 1 ' .
            ' WHEN ' . Forum::STICKY . ' THEN 1 ' .
            ' WHEN ' . Forum::STICKY_ONLY_CATEGORY . ' THEN 1 ' .
            ' WHEN ' . Forum::STICKY_ONLY_CATEGORY_CLOSED . ' THEN 1 ' .
            ' ELSE 0 END DESC, ' .
            't.last_post_time DESC LIMIT ' . $num_posts . ' OFFSET ' . $from;
    ...
    private function getLatestTopics($from = 0)
    {
        ...
        $qry = 'SELECT  p.reputation,p.post_id, p.omessage AS message, p.post_created, r.rid, u.id, ' .
            'u.name as name, u.avatar, c.cat_id, c.cat_img, c.cat_alias, c.cat_name, ' .
    ...
    public function get_catid_title_tuid($topic_id)
    {
        ...
        $qry = 'SELECT cat_id,title,uid AS tuid,topic_status FROM ' . PREFIX . 'codo_topics WHERE topic_id=' . $topic_id;
    ...
    /**
     * Compares available status to current status to return if topic is closed.
     * @param $topicStatus
     * @return bool
     */
    public function isTopicClosed($topicStatus)
    {
        return $topicStatus == \CODOF\Forum\Forum::APPROVED_CLOSED ||
            $topicStatus == \CODOF\Forum\Forum::STICKY_CLOSED || $topicStatus == \CODOF\Forum\Forum::STICKY_ONLY_CATEGORY_CLOSED;
    }
    ...
    public function gen_topic_arr_all_topics($topics, $search = false)
    {
        ...
        foreach ($topics as $topic) {
            ...
            $_topics [$i] = array(
                ...
                "cat_name" => isset($topic ['cat_name']) ? $topic ['cat_name'] : "",
            ...
            $excerpt = \CODOF\Format::excerpt($message);

            if ($search) {
                $_topics [$i] ["is_topic"] = $topic['topic_post_id'] == $topic['post_id'];
                $_topics[$i]["cat_name"] = $topic['cat_name'];
                $excerpt ['message'] = $search->get_matching_str($excerpt ['message']);
                // Using DOMDocument to tidy up invalid HTML
                $doc = new \DOMDocument();
                @$doc->loadHTML($excerpt ['message']);
                $excerpt ['message'] = $doc->saveHTML();
            }
    ...
    /**
     * Checks if particular topic can be replied by current user or not
     *
     * @param int $tuid
     *            topic creator's userid
     * @param int $cid
     ...
     * @return bool
     */
    public function canReplyTopic($tuid, $cid, $tid, $topicStatus)
    {
        if ($this->isTopicClosed($topicStatus))
            return false;
