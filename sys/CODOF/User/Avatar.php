...
class Avatar {
	...
	public static $heightWidthImage = 256;
	...
	public function generate($id, $display = true) {
        ...
        $heightWidthIcon = self::$heightWidthImage/2;

		// Create a 300x100 image
		$im = imagecreatetruecolor ( self::$heightWidthImage, self::$heightWidthImage );
		$red = imagecolorallocate ( $im, $rgb [0], $rgb [1], $rgb [2] );
		$black = imagecolorallocate ( $im, 0xFF, 0xFF, 0xFF );
		
		// Make the background red
		imagefilledrectangle ( $im, 0, 0, self::$heightWidthImage, self::$heightWidthImage, $red );
		
		// Path to our ttf font file
		$font_file = ASSET_DIR . 'img/general/SourceCodePro-Light.ttf';
        $fontToImageGap = 64;
        $fontSize = self::$heightWidthImage - $fontToImageGap;
		imagefttext ( $im, $fontSize, 0, 48, 208, $black, $font_file, $initial );
		...
		$iconW = $heightWidthIcon;
		$iconH = $heightWidthIcon;
