...
class Html {
    ...
    private static $whitelist = array(
        'p' => array('align'),
        'div' => array('align', 'class', 'data-pid', 'data-uid'),
        'span' => array('class'),
        'font' => array('color'),
        'br' => array(),
        'details' => array(),
        ...
        'a' => array('href', 'target', 'class', 'title', 'data-pid'),
        'video' => array('src', 'controls', 'width'),
        'img' => array('src',
            'alt', 'title',
        ),
    ...
    public function filter($val, $strict = false, $import = false) {
        ...
        $this->cleanNodes($body, $strict, $badTags);
    ...
    protected function addIWithAllowedClass(&$node) {
        ...
        if ($node->hasAttribute('class')) {
            ...
            $cls = explode(" ", $node->getAttribute('class'));
            $allowed_cls = array('glyphicon', 'glyphicon-file', 'icon', 'icon-arrow-top');
    ...
    /**
     * Allow a class for <div> tag
     *
     * @param [type] $node
     * @return void
     */
    protected function addDivWithAllowedClassAndDataAttributes(&$node) {

        if ($node->hasAttribute('class')) {

            $cls = explode(" ", $node->getAttribute('class'));
            $allowed_cls = array('codo_quote_container');
     ...
     */
    protected function cleanNodes($node, $strict, &$badTags = array()) {
        ...
        if (array_key_exists($node->nodeName, self::$whitelist)) {
            ...
            if ($node->hasAttributes()) {
                ...
                if('div' === $node->nodeName) {
                    $this->addDivWithAllowedClassAndDataAttributes($node);
                }
            ...
            if ($node->childNodes) {
                foreach ($node->childNodes as $child) {
                    $this->cleanNodes($child, $strict, $badTags);
                }
            }
        } else {
