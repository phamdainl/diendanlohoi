...
use CODOF\File\Upload;
use CODOF\Filter;
use CODOF\Format;
use CODOF\Forum\Forum;
use CODOF\Forum\Notification\Notifier;
use CODOF\Forum\Notification\Subscriber;
use CODOF\Hook;
use CODOF\Search\Search;
use CODOF\SpamFilter;
...
class topic
{
    ...
    public function get_posts(int $tid, $from, $topic_info)
    {
        ...
        $posts_per_page = Util::get_opt("num_posts_per_topic");
        $title = Filter::URL_safe($topic_info['title']);
        ...
        if (isset($_GET['str']) && $_GET['str'] != "") {
            ...
            $search->order = "DESC";
            $search->sort = "post_created";
            $search->time_within = "anytime";
    ...
    public function reply()
    {
        ...
        if (!$topic->canViewTopic($info['tuid'], $catid, $tid) ||
            !$topic->canReplyTopic($info['tuid'], $catid, $tid, $info['topic_status'])) {
        ...
        if (isset($_POST['input_txt']) && isset($_POST['output_txt']) && isset($_POST['tid'])) {
            ...
            if (!$subscriber->existsForTopic($catid, $tid, $user->id)) {
                ...
                $subscriber->toTopic($catid, $tid, Subscriber::$NOTIFIED);
            }
            ...
            if ($pid) {
                ...
                $topicData = array(
                    ...
                    "message" => Util::start_cut(Format::imessage($_POST['input_txt']), 120), //OPTIONAL
                    "mentions" => $ids,
                    "notification" => "%actor% replied to <b>%title%</b>",
                    "bindings" => array("title" => Util::start_cut($title, 100))
                );

                $notifier->queueNotify('new_reply', $topicData, $tid);
                Hook::call('after_reply_insert', $topicData);
            }
    ...
    public function upload()
    {
        ...
        if (is_array($_FILES['file']['name'])) {
            $images = Util::re_array_files($_FILES['file']);
        } else {
        ...
        foreach ($images as $image) {
            ...
            } else if (!Upload::not_empty($image)) {
                $errors[] = "Image is empty";
            } else if (!Upload::size($image, (int)Util::get_opt('forum_attachments_size'))) {
                $errors[] = "Image size " . $image['size'] . " too large. Limit: " . Util::get_opt('forum_attachments_size') * 1024 * 1024;
            } else if (!Upload::type($image, explode(",", Util::get_opt('forum_attachments_exts')))) {
                $errors[] = "Inavlid image type";
            } else {
    ...
    public function create()
    {
        ...
        if (isset($_POST['title']) && isset($_POST['cat']) && isset($_POST['imesg']) && isset($_POST['omesg'])) {
            ...
            Hook::call('before_topic_insert', [$_POST['title'], $_POST['imesg'], $tags]);
            ...
            $title = Format::title($_POST['title']);
            ...
            if ($sticky == 'stickyfc' && $user->can('make sticky')) {
                ...
                if ($open == 'no' && $user->can('close topics')) {
                    ...
                    $tid = $topic->ins_topic($catid, $title, $needsModeration, Forum::STICKY_CLOSED);
                } else {
                    $tid = $topic->ins_topic($catid, $title, $needsModeration, Forum::STICKY);
                }
            } else if ($sticky == 'stickyc' && $user->can('make sticky')) {
                if ($open == 'no' && $user->can('close topics')) {

                    $tid = $topic->ins_topic($catid, $title, $needsModeration, Forum::STICKY_ONLY_CATEGORY_CLOSED);
                } else {

                    $tid = $topic->ins_topic($catid, $title, $needsModeration, Forum::STICKY_ONLY_CATEGORY);
                }
            } else {

                if ($open == 'no' && $user->can('close topics')) {

                    $tid = $topic->ins_topic($catid, $title, $needsModeration, Forum::APPROVED_CLOSED);
                } else {
                    $tid = $topic->ins_topic($catid, $title, $needsModeration, Forum::APPROVED);
                }
            }
            ...
            if ($pid) {
                ...
                $topicData = array(
                    ...
                    "link" => 'topic/' . $tid . '/post-' . $pid . '&page=from_notify&nid=[NID]/#post-' . $pid,
                    "notifyFrom" => CurrentUser::id(),
                    "topicCreatorUid" => CurrentUser::id(),
                    "message" => Util::start_cut(Format::imessage($_POST['imesg']), 120),
                ...
                Hook::call('after_topic_insert', $topicData);
            }
    ...
    public function edit()
    {
        ...
        if ($has_permission) {
            ...
            if (isset($_POST['title']) && isset($_POST['cat']) && isset($_POST['imesg']) && isset($_POST['omesg'])) {
                ...
                Hook::call('before_topic_edit', [$_POST['title'], $_POST['imesg'], $tags]);
                ...
                if ($topicNeedsToBeMoved) {
                    ...
                    if ($_POST['notify'] === 'true') {
                        ...
                        $topicData = array(
                            ...
                            "bindings" => array("title" => Util::start_cut($topic_info['title'], 100),
                ...
                if ($sticky == 'stickyfc' && $user->can('make sticky')) {
                    ...
                    if ($open == 'no' && $user->can('close topics')) {
                        ...
                        $new_topic_status = Forum::STICKY_CLOSED;
                    } else {
                        $new_topic_status = Forum::STICKY;
                    }
                } else if ($sticky == 'stickyc' && $user->can('make sticky')) {
                    if ($open == 'no' && $user->can('close topics')) {

                        $new_topic_status = Forum::STICKY_ONLY_CATEGORY_CLOSED;
                    } else {

                        $new_topic_status = Forum::STICKY_ONLY_CATEGORY;
                    }
                } else {

                    if ($open == 'no' && $user->can('close topics')) {

                        $new_topic_status = Forum::APPROVED_CLOSED;
                    } else {
                        $new_topic_status = Forum::APPROVED;
                    }
                }
    ...
    public function merge($tids, $dest_tid)
    {
        ...
        DB::table(PREFIX . 'codo_topics')
            ...
            ->update(array(
                'redirect_to' => $dest,
                'topic_status' => Forum::MERGED_REDIRECT_ONLY));
