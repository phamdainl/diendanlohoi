...
CODOF.hook.add('on_oembed_loaded', function () {
    ...
    const gutter = 20; // some offset for edge cases
    $(".codo_topics_topic_message").each(function (index) {
        if ($(this)[0].scrollHeight > ($(this)[0].offsetHeight + gutter)) {
            $(this).next().find('.codo_readmore').show();
        }
    });
});
...
jQuery('document').ready(function ($) {
                    ...
                    $.getJSON(
                        ...
                        function (response) {
                            ...
                            if (response.num_posts) {
                                ...
                                CODOF.topics.from += parseInt(response.num_posts);
    ...
    CODOF.changePage = function (el, currPage, action) {
        ...
        /*
        V5.0 -> Moved search from topics page into an independent page
        var searchStr = '';
        ...
        window.location.href = codo_defs.url + 'topics/' + nextPage/* + searchStr*/;
    };
