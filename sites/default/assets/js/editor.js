...
CODOF.editor_settings = {
    ...
    getVideoHtml: function(src, title, text) {
        return '<video preload="none" width="400px" src="' + codo_defs.url + src + '" controls="true"></video>';
    },
    ...
    previewParser: function (content) {
        ...
        renderer.link = function (href, title, text) {
            ...
            //if ends with some img extension, image intended
            if (/\.(?:mp4|mov|avi|mkv|webm)$/i.test(text)) {
                //may be an image
                return editor.getVideoHtml(href, title);
            }
        ...
        $('#codomyawesomedropzone').dropzone({
            ...
            init: function () {
                ...
                dz.on("success", function (file, response) {
                    ...
                    if(response.error) {
                        CODOF.util.alert(response.error, "Failed to upload file");
                        return false;
                    }
