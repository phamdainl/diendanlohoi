...
class Notify
{
     ...
     * TODO: ASSUMES notfication_frequency has default value as immediate. IF in future this is
     * made configurable then the query will have to be modified
     *
     * @param $cid
     * @param $tid
     * @param $pid
    ...
    public function getCategoryOrTopicData($cid, $tid, $pid, $offset, $type, $notifyFrom)
    {
        $defaultSubscription = \DB::table(PREFIX . 'codo_categories')
            ...
            ->where('cat_id', '=', $cid)->value(1);

        if ($defaultSubscription == \CODOF\Forum\Notification\Subscriber::$NOTIFIED) {
            // all users except those who explicitly have unsubscribed must be notified :(
            $data = \DB::table(PREFIX . 'codo_users AS u')
                ...
                ->whereExists(function ($query){
                    $query->select(\DB::raw(1))
                        ...
                        ->whereRaw('perm.cid = c.cat_id')
                        ->whereIn('perm.rid', function($inQuery) {
                ...
                ->where('t.topic_status', '<>', \DB::raw(\CODOF\Forum\Forum::DELETED))
                ->whereNotExists(function ($query) {
                    $query->select(\DB::raw(1))
                        ->from(PREFIX . 'codo_user_preferences AS pref')
                        ->whereRaw('pref.uid = u.id')
                        ->whereRaw('pref.preference = \'notification_frequency\'');
                })
    ...
    public function getData($cid, $tid, $pid, $offset, $type, $notifyFrom)
    {
        ...
        $data = \CODOF\Util::toAssociativeArray($data);
        $mailData = [];
        ...
        while ($data = $this->getData($cid, $tid, $pid, $offset, $type, $notifyFrom)) {
            ...
            foreach ($data as $info) {
                ...
                $post = array(
                    ...
                    "url" => \CODOF\Forum\Forum::getPostURL($tid, html_entity_decode($info['title']), $pid),
                    "id" => $info['id'],
