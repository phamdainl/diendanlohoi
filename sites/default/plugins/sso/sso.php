...
dispatch_get('/sso/authorize', function () {
    \CODOF\Store::set('sub_title', _t('Authenticating'));
    \CODOF\Smarty\Layout::load("sso:sso");
    add_sso_defs();
    add_sso_js();
});
...
dispatch_post('/sso/authorize', function () {
    ...
    //CSRF protection
    if (CODOF\Access\CSRF::valid($_POST['token'])) {
...
function add_sso_defs()
{
    ...
    $sso_login_success_url = User::getLoginSuccessRedirectUrl();
    $can_view_forum = User::get()->can('view forum') ? 'yes' : 'no';
    ...
    $html = <<<EOD
        ...
        <div id="_codo_sso_login_success_url">$sso_login_success_url</div>
        <div id="_codo_can_view_forum">$can_view_forum</div>
