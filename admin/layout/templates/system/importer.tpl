...
<div class="row">
  ...
  <div class="col-md-12">
    ...
    <div class="warn">Note: All your forum categories, topics, posts, attachments(from filesystem) and users will be deleted/overwritten!</div>
    ...
    <form id="codo_importer" role="form" method="POST" class="form form-horizontal">
        ...
        <fieldset>
            ...
            <div id="vbulletin_attachments_dir_div" class="form-group row" style="display: none">
                <label  class="col-sm-2 control-label" for="name">Absolute path to vBulletin attachments directory</label>
                <div class="col-sm-8">
                    <input type="text" class="form-control" id="vbulletin_attachments_dir" placeholder="Enter absolute path">
                </div>
            </div>
...
<script type="text/javascript">
    ...
    function doBulkImport(i) {
        $.get('{$home}Ajax/cron/run/import_bulk_data', {
            token: '{$token}'
        }, function (data) {
            if (data === '') {
                $('#codo_import_status').append("<br/>Copying batch no. " + i);
                setTimeout(() => {
                    doBulkImport(i+1);
                }, 1000);
            } else {
                $('#codo_import_status').append(`<br/>Import completed!`);
            }
        });
    }
    ...
    function processStep(step) {
        ...
        $.get('index.php?page=system/importer&import=yes', {
            ...
            vbulletin_attachments_dir: $('#vbulletin_attachments_dir').val(),
            import_step: step,
            CSRF_token: "{$token}"
        }, function(response) {

            $('#codo_import_status').append(response).show();
            const forum = $('#import_from').val();
            ...
            if (response === 'Unable to connect to database' || response === 'admin e-mail address given does not exists!') {
                ...
                processStep(step+1);
            } else if (forum === "VBulletin") {
                $('#codo_import_status').append("<br/>Found attachments to be copied...please wait.");
                doBulkImport(1);
            }
        });
    ...
    jQuery('document').ready(function($) {
        ...
        $('#import_from').on('change', function (el) {
            const forum = $('#import_from').val();

            if (forum == "VBulletin") {
                $('#vbulletin_attachments_dir_div').show();
            } else {
                $('#vbulletin_attachments_dir_div').hide();
            }
        });
