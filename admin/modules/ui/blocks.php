...
class A_Block {
    ...
    function get_tpl_contents($theme = 'default') {
        ...
        if ($info['parent_theme'] != 'default') {
            $pfiles = glob_recursive(DATA_PATH . 'themes/default/*.tpl');
        }
...
// $blocks=  array_merge($blocks,$matches);
    ...
    function list_blocks() {
        $themes = \CODOF\Util::getThemesByInhereitance();
        $in  = str_repeat('?,', count($themes) - 1) . '?';
        $query = "SELECT * FROM " . PREFIX . "codo_blocks WHERE theme IN ($in)";
...
if (isset($_GET['action'])) {
    ...
    if ($_GET['action'] == 'add') {
        ...
        if (isset($_POST['mode']) && CODOF\Access\CSRF::valid($_POST['CSRF_token'])) {
            ...
            https://www.saferack.com/wp-content/uploads/2017/03/IMG_00015.jpg
            if ($_POST['mode'] == 'add') {
