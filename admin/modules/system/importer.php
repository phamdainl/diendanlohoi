...
if (isset($_GET['import']) && CODOF\Access\CSRF::valid($_GET['CSRF_token'])) {
    ...
    $work->customData['vbulletin_attachments_dir'] = $_GET['vbulletin_attachments_dir'];
    ...
    if ($work->connected && $is_admin) {
        ...
        switch ($step) {
            ...
            case 1:
                ...
                $path = DATA_PATH . \CODOF\Util::get_opt('forum_attachments_path');
                deleteFiles(rtrim($path, '/') . '/preview/*');
                deleteFiles(rtrim($path, '/') . '/*');
                $work->import_cats();
