...
class upgrader {
    ...
    static function check_latest() {
        ...
        echo "####: Checking PHP requirement<br>";
        $req_php_version = self::get_min_php();
        if (phpversion() < $req_php_version) {
            echo "[ERROR] You need at least <b>PHP $req_php_version</b> to upgrade to the next version"; return;
        } else {
        ...
        $curl->get(upgrader::$base_url . 'news/v2/latest.php');
    ...
    static function get_min_php() {
        $curl = new Curl\Curl();
        $curl->setOpt(CURLOPT_SSL_VERIFYPEER, false);
        //$curl_setopt(CURLOPT_SSLVERSION , 3);
        $curl->get(upgrader::$base_url . 'news/req.php');
